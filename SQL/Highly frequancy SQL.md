...
1. **定义函数的名称**：每个函数都有一个唯一的名称。
2. **定义参数**：可以定义输入参数。
3. **定义返回类型**：指定函数返回值的数据类型。
4. **定义返回参数**： 定义输出的参数。
5. **编写函数体**：包含实际的PL/SQL代码和SQL语句。
6. **异常处理**：处理在执行过程中可能发生的异常。
...
BEGIN
    ...
    select distinct nvl(salary,null) into result 
    from (select salary, dense_rank() over (order by salary desc) ranks from employee )
    where ranks = N;
...
DECLARE
    result NUMBER;
BEGIN
    result := getNthHighestSalary(2);
    DBMS_OUTPUT.PUT_LINE('getNthHighestSalary: ' || result);
END;
```
...
```sql
SELECT getNthHighestSalary(2) AS getNthHighestSalary
FROM dual;
```
...
with 
cancelled_trip_count as (
    select request_at, count(id) cancelled_num
    from trips
    where (status = 'cancelled_by_driver' or status = 'cancelled_by_client') 
    and client_id in (select users_id no_banned_client_id from users where role = 'client' and banned = 'No')
    and driver_id in (select users_id driver_id from users where role = 'driver' and banned = 'No')
    and request_at >= '2013-10-01' and request_at  <= '2013-10-03'
    group by request_at),
perday_trips as(
    select request_at, count(id) per_day_trips
    from trips
    where client_id in (select users_id no_banned_client_id from users where role = 'client' and banned = 'No')
    and driver_id in (select users_id driver_id from users where role = 'driver' and banned = 'No')
    and request_at >= '2013-10-01' and request_at  <= '2013-10-03'
    group by request_at)

select t.request_at day, round(nvl(c.cancelled_num,0)/t.per_day_trips,2) "cancellation rate"
from perday_trips t
left join cancelled_trip_count c
...
round(sum(case when status= 'cancelled_by_driver' or status = 'cancelled_by_client' then 1 else 0 end)/count(*),2) "cancellation rate"
from trips
